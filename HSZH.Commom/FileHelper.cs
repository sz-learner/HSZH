...
namespace HSZH.Commom
{
    ...
    {
        private static string CurrAppPath = "";
        ...
        /// <param name="IsWeb">true web应用程序 false 桌面应用程序</param>
        public static void CreateDir(string dir, bool IsWeb = true)
        {
            ...
            try
            {
                ...
                {
                    string filepath = Path.Combine(GetLocalPath(), dir);
        ...
        /// <param name="IsWeb">true web应用程序 false 桌面应用程序</param>
        public static void DeleteDir(string dir, bool IsWeb = true)
        {
            if (dir.Length == 0) return;
            try
            {
                ...
                {
                    if (Directory.Exists(System.Web.HttpContext.Current.Request.PhysicalApplicationPath + "\\" + dir))
                        Directory.Delete(System.Web.HttpContext.Current.Request.PhysicalApplicationPath + "\\" + dir, true);
                }

                else
                {
                    if (Directory.Exists(Path.Combine(GetLocalPath(), dir)))
                        Directory.Delete(Path.Combine(GetLocalPath(), dir), true);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="IsWeb">true web应用程序 false 桌面应用程序</param>
        public static void DeleteFile(string file, bool IsWeb = true)
        {
            try
            {
                if (IsWeb)
                ...
                {
                    if (File.Exists(Path.Combine(GetLocalPath(), file)))
                        File.Delete(Path.Combine(GetLocalPath(), file));
                }
            }
            catch (Exception ex)
            {
        ...
        public static void MoveFile(string dir1, string dir2, bool IsWeb = true)
        {
            dir1 = dir1.Replace("/", "\\");
            dir2 = dir2.Replace("/", "\\");
            if (IsWeb)
            ...
            {
                if (File.Exists(Path.Combine(GetLocalPath(), dir1)))
                    File.Move(Path.Combine(GetLocalPath(), dir1), Path.Combine(GetLocalPath(), dir2));
            }
        }
        #endregion
        ...
        public static void CopyFile(string dir1, string dir2, bool IsWeb = true)
        {
            dir1 = dir1.Replace("/", "\\");
            dir2 = dir2.Replace("/", "\\");
            if (IsWeb)
            ...
            else
            {
                if (File.Exists(Path.Combine(GetLocalPath(), dir1)))
                {
                    File.Copy(Path.Combine(GetLocalPath(), dir1), Path.Combine(GetLocalPath(), dir2), true);
                }
            }
        }
        #endregion
        ...
        public static string ReadFileContent(string fileName)
        {
            if (!File.Exists(fileName))
                throw new FileNotFoundException(string.Format("FileNotFound:{0}", fileName));
        ...
        public static string GetLocalPath()
        {
            ...
            str = str.Replace("/", "\\");
            if (CurrAppPath.IsEmpty())
                CurrAppPath = str;
            else
            {
                if (!CurrAppPath.Equals(str))
                    return CurrAppPath;
            }
            return str;
        }
