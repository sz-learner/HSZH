...
namespace HSZH.Hardware.FaceCheck
{
    ...
    public class FaceEngine
    {
        ...
        public int ASFGetActiveFileInfo(out ActiveFileInfo activeFileInfo)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetActiveFileInfo(pASFActiveFileInfo);
            if (retCode != 0)
            {
        ...
        {
            return HongRuanFaceCheckDll.ASFOnlineActivation(appId, appKey, activeKey);
        }
        ...
        public int ASFGetActiveDeviceInfo(out string deviceInfo)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetActiveDeviceInfo(ref tempIntPtr);
            if (retCode == 0)
            {
        ...
        {
            return HongRuanFaceCheckDll.ASFOfflineActivation(filePath);
        }
        ...
        public int ASFInitEngine(DetectionMode detectMode, ASF_OrientPriority detectFaceOrientPriority, int detectFaceMaxNum, int combinedMask)
        {
            ...
            return HongRuanFaceCheckDll.ASFInitEngine(detectMode, detectFaceOrientPriority, detectFaceMaxNum, combinedMask, ref pEngine);
        }
        ...
        {
            return HongRuanFaceCheckDll.ASFSetFaceShelterParam(pEngine, shelterThreshhold);
        }
        ...
        public int ASFDetectFaces(Image image, out MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB, bool isEx = false)
        {
            ...
            if (isEx)
            {
                ...
                retCode = HongRuanFaceCheckDll.ASFDetectFacesEx(pEngine, pImageInfo, pMultiFaceInfo);
                MemoryUtil.Free(pImageInfo);
            }
            else
            {
                retCode = HongRuanFaceCheckDll.ASFDetectFaces(pEngine, imageInfo.width, imageInfo.height, imageInfo.format, imageInfo.imgData, pMultiFaceInfo);
            }
        ...
        public int ASFUpdateFaceData(Image image, ref MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            ...
            if (isEx)
            {
                ...
                IntPtr pImageInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_ImageData>());
                MemoryUtil.StructureToPtr(asfInfoData, pImageInfo);
                retCode = HongRuanFaceCheckDll.ASFUpdateFaceDataEx(pEngine, pImageInfo, pMultiFaceInfo);
                MemoryUtil.Free(pImageInfo);
            }
            else
            {
                retCode = HongRuanFaceCheckDll.ASFUpdateFaceData(pEngine, imageInfo.width, imageInfo.height, imageInfo.format, imageInfo.imgData, pMultiFaceInfo);
            }
            if (retCode != 0)
            {
        ...
        public int ASFImageQualityDetect(Image image, MultiFaceInfo multiFaceInfo, out float confidenceLevel, int faceIndex = 0, bool isMask = false, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB, bool isEx = false)
        {
            ...
            if (isEx)
            {
                ...
                IntPtr pImageInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_ImageData>());
                MemoryUtil.StructureToPtr(asfInfoData, pImageInfo);
                retCode = HongRuanFaceCheckDll.ASFImageQualityDetectEx(pEngine, pImageInfo, pSingleFaceInfo, isMask ? ASF_MaskOrNot.ASF_MASK : ASF_MaskOrNot.ASF_NOT, ref confidenceLevel, detectModel);
                MemoryUtil.Free(pImageInfo);
            }
            else
            {
                retCode = HongRuanFaceCheckDll.ASFImageQualityDetect(pEngine, imageInfo.width, imageInfo.height, imageInfo.format, imageInfo.imgData, pSingleFaceInfo, isMask ? ASF_MaskOrNot.ASF_MASK : ASF_MaskOrNot.ASF_NOT, ref confidenceLevel, detectModel);
            }
        ...
        public int ASFProcess(Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            ...
            if (isEx)
            {
                ...
                IntPtr pImageInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_ImageData>());
                MemoryUtil.StructureToPtr(asfInfoData, pImageInfo);
                retCode = HongRuanFaceCheckDll.ASFProcessEx(pEngine, pImageInfo, pMultiFaceInfo, combinedMask);
                MemoryUtil.Free(pImageInfo);
            }
            else
            {
                retCode = HongRuanFaceCheckDll.ASFProcess(pEngine, imageInfo.width, imageInfo.height, imageInfo.format, imageInfo.imgData, pMultiFaceInfo, combinedMask);
            }
            //释放内存
        ...
        public int ASFProcess_IR(Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_GRAY, bool isEx = false)
        {
            ...
            if (isEx)
            {
                ...
                IntPtr pImageInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_ImageData>());
                MemoryUtil.StructureToPtr(asfInfoData, pImageInfo);
                retCode = HongRuanFaceCheckDll.ASFProcessEx_IR(pEngine, pImageInfo, pMultiFaceInfo, combinedMask);
                MemoryUtil.Free(pImageInfo);
            }
            else
            {
                retCode = HongRuanFaceCheckDll.ASFProcess_IR(pEngine, imageInfo.width, imageInfo.height, imageInfo.format, imageInfo.imgData, pMultiFaceInfo, combinedMask);
            }
            //释放内存
            MemoryUtil.FreeArray(imageInfo.imgData, multiFaceInfoStruct.faceDataInfoList, multiFaceInfoStruct.faceID, multiFaceInfoStruct.faceOrient, multiFaceInfoStruct.faceRect, multiFaceInfoStruct.faceShelter, multiFaceInfoStruct.leftEyeClosed, multiFaceInfoStruct.rightEyeClosed, multiFaceInfoStruct.wearGlasses, pMultiFaceInfo);
            MemoryUtil.FreeList(faceDataInfoIntPtrList);
        ...
        public int ASFFaceFeatureExtract(Image image, MultiFaceInfo multiFaceInfo, ASF_RegisterOrNot registerOrNot, out FaceFeature faceFeature, int faceIndex = 0, bool isMask = false, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            ...
            if (isEx)
            {
                ...
                IntPtr pImageInfo = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_ImageData>());
                MemoryUtil.StructureToPtr(asfInfoData, pImageInfo);
                retCode = HongRuanFaceCheckDll.ASFFaceFeatureExtractEx(pEngine, pImageInfo, pSingleFaceInfo, registerOrNot, isMask ? ASF_MaskOrNot.ASF_MASK : ASF_MaskOrNot.ASF_NOT, pAsfFaceFeature);
                MemoryUtil.Free(pImageInfo);
            }
            else
            {
                retCode = HongRuanFaceCheckDll.ASFFaceFeatureExtract(pEngine, imageInfo.width, imageInfo.height, imageInfo.format, imageInfo.imgData, pSingleFaceInfo, registerOrNot, isMask ? ASF_MaskOrNot.ASF_MASK : ASF_MaskOrNot.ASF_NOT, pAsfFaceFeature);
            }
            if (retCode == 0)
            {
                //获取特征结构体，并转化
        ...
        public int ASFFaceFeatureCompare(FaceFeature faceFeature1, FaceFeature faceFeature2, out float similarity, ASF_CompareModel compareModel = ASF_CompareModel.ASF_LIFE_PHOTO)
        {
            ...
            #endregion
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFFaceFeatureCompare(pEngine, pFeature1, pFeature2, ref similarity, compareModel);
        ...
        public int ASFGetAge(out AgeInfo ageInfo)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetAge(pEngine, pAgeInfo);
            if (retCode != 0)
            {
        ...
        public int ASFGetGender(out GenderInfo genderInfo)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetGender(pEngine, pGenderInfo);
            if (retCode != 0)
            {
        ...
        public int ASFGetFace3DAngle(out Face3DAngle faceAngle)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetFace3DAngle(pEngine, pFaceAngle);
            if (retCode != 0)
            {
        ...
        public int ASFSetLivenessParam(float rgbThreshold = 0.5f, float irThreshole = 0.7f)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFSetLivenessParam(pEngine, pLivenessThreshold);
        ...
        public int ASFGetLivenessScore(out LivenessInfo livenessInfo)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetLivenessScore(pEngine, pLiveness);
            if (retCode != 0)
            {
        ...
        public int ASFGetLivenessScore_IR(out LivenessInfo livenessInfo)
        {
            ...
            IntPtr pLiveness = MemoryUtil.Malloc(MemoryUtil.SizeOf<ASF_LivenessInfo>());
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetLivenessScore_IR(pEngine, pLiveness);
            if (retCode != 0)
            {
                MemoryUtil.Free(pLiveness);
                return retCode;
        ...
        public int ASFGetMask(out MaskInfo maskInfo)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetMask(pEngine, pMaskInfo);
            if (retCode != 0)
            {
        ...
        public int ASFGetFaceLandMark(out LandMarkInfo faceLandMark)
        {
            ...
            //调用SDK接口
            retCode = HongRuanFaceCheckDll.ASFGetFaceLandMark(pEngine, pFaceLandMark);
            if (retCode != 0)
            {
        ...
        public void ASFUninitEngine()
        {
            try
            {
                ...
                {
                    HongRuanFaceCheckDll.ASFUninitEngine(pEngine);
                }
            }
            catch { }
        }
        ...
        public void ASFGetVersion(out SDKVersion version)
        {
            ...
            ASF_VERSION asfVersion = HongRuanFaceCheckDll.ASFGetVersion();
